...
export const Header = () => {
  ...
  return (
    <>
      <header className='h-24 flex justify-end items-center p-5 fixed w-screen z-50 bg-white z-50 md:static md:flex-col md:justify-center md:h-auto md:p-8 md:pb-4'>
        ...
        <ul className={`${s.header_menu} ${isOpen ? s.is_open : ''} `}>
          {navItems.map((item, index) => (
            <li className={`${s.item} ${isOpen ? s.appear : ''}`} style={{ transitionDelay: `${100 + index * 100}ms` }} key={item.name + index}>
              <Link className='text-text-black text-2xl font-display font-bold duration-300 lg:text-3xl md:hover:-translate-y-1' href={`/${item.link}`} onClick={closeMenu}>
                {item.name}
              </Link>
            </li>
          ))}
